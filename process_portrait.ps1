# Fully Bulletproof ASCII Banner Generator with Clean Crisp Fonts & Zero Symbol Corruptions

Add-Type -AssemblyName System.Drawing

$photoPath = "C:\Users\Lenovo\.gemini\antigravity-ide\brain\d07fd918-4a57-4e65-a308-546ab18877d4\media__1786041519359.jpg"
$darkSvgPath = "d:\github readme\dark.svg"
$lightSvgPath = "d:\github readme\light.svg"

# 1. Load Image and crop head/shoulders
$srcImg = [System.Drawing.Image]::FromFile($photoPath)
$srcWidth = $srcImg.Width
$srcHeight = $srcImg.Height

$targetAspect = 300.0 / 340.0
$srcAspect = [double]$srcWidth / [double]$srcHeight

if ($srcAspect -gt $targetAspect) {
    $cropW = [int]($srcHeight * $targetAspect)
    $cropH = $srcHeight
    $cropX = [int](($srcWidth - $cropW) / 2)
    $cropY = 0
} else {
    $cropW = $srcWidth
    $cropH = [int]($srcWidth / $targetAspect)
    $cropX = 0
    $cropY = [int](($srcHeight - $cropH) * 0.05)
}

$cropRect = New-Object System.Drawing.Rectangle($cropX, $cropY, $cropW, $cropH)
$gridW = 100
$gridH = 113
$smallBmp = New-Object System.Drawing.Bitmap($gridW, $gridH)
$g = [System.Drawing.Graphics]::FromImage($smallBmp)
$g.InterpolationMode = [System.Drawing.Drawing2D.InterpolationMode]::HighQualityBicubic
$g.DrawImage($srcImg, (New-Object System.Drawing.Rectangle(0, 0, $gridW, $gridH)), $cropRect, [System.Drawing.GraphicsUnit]::Pixel)
$g.Dispose()
$srcImg.Dispose()

# 2. Extract luminance array and apply autocontrast x1.3
$gray = New-Object 'double[]' ($gridH * $gridW)
$minL = 255.0
$maxL = 0.0

for ($y = 0; $y -lt $gridH; $y++) {
    for ($x = 0; $x -lt $gridW; $x++) {
        $idx = ($y * $gridW) + $x
        $c = $smallBmp.GetPixel($x, $y)
        $l = 0.299 * $c.R + 0.587 * $c.G + 0.114 * $c.B
        $gray[$idx] = $l
        if ($l -lt $minL) { $minL = $l }
        if ($l -gt $maxL) { $maxL = $l }
    }
}
$smallBmp.Dispose()

$range = [math]::Max(1.0, ($maxL - $minL))
for ($i = 0; $i -lt ($gridH * $gridW); $i++) {
    $norm = ($gray[$i] - $minL) / $range
    $cVal = 0.5 + ($norm - 0.5) * 1.3
    $cVal = [math]::Max(0.0, [math]::Min(1.0, $cVal))
    $gray[$i] = $cVal * 255.0
}

# 3. Floyd-Steinberg Dithering with Serpentine Scan Order
$dither = New-Object 'int[]' ($gridH * $gridW)
$errBuf = New-Object 'double[]' ($gridH * $gridW)

for ($i = 0; $i -lt ($gridH * $gridW); $i++) {
    $errBuf[$i] = $gray[$i]
}

for ($y = 0; $y -lt $gridH; $y++) {
    $leftToRight = ($y % 2 -eq 0)
    $startX = if ($leftToRight) { 0 } else { $gridW - 1 }
    $endX = if ($leftToRight) { $gridW - 1 } else { 0 }
    $step = if ($leftToRight) { 1 } else { -1 }

    $x = $startX
    while ($true) {
        $idx = ($y * $gridW) + $x
        $oldPixel = $errBuf[$idx]
        $newPixel = if ($oldPixel -lt 120.0) { 0 } else { 255 }
        $dither[$idx] = $newPixel
        $err = $oldPixel - $newPixel

        if ($leftToRight) {
            if ($x + 1 -lt $gridW) { $errBuf[$idx + 1] += $err * (7.0 / 16.0) }
            if ($y + 1 -lt $gridH) {
                $nextRow = ($y + 1) * $gridW
                if ($x - 1 -ge 0) { $errBuf[$nextRow + $x - 1] += $err * (3.0 / 16.0) }
                $errBuf[$nextRow + $x] += $err * (5.0 / 16.0)
                if ($x + 1 -lt $gridW) { $errBuf[$nextRow + $x + 1] += $err * (1.0 / 16.0) }
            }
        } else {
            if ($x - 1 -ge 0) { $errBuf[$idx - 1] += $err * (7.0 / 16.0) }
            if ($y + 1 -lt $gridH) {
                $nextRow = ($y + 1) * $gridW
                if ($x + 1 -lt $gridW) { $errBuf[$nextRow + $x + 1] += $err * (3.0 / 16.0) }
                $errBuf[$nextRow + $x] += $err * (5.0 / 16.0)
                if ($x - 1 -ge 0) { $errBuf[$nextRow + $x - 1] += $err * (1.0 / 16.0) }
            }
        }

        if ($x -eq $endX) { break }
        $x += $step
    }
}

# 4. Extract Dark Dither Dots
$portraitPoints = New-Object System.Collections.Generic.List[System.Drawing.PointF]

for ($y = 0; $y -lt $gridH; $y++) {
    for ($x = 0; $x -lt $gridW; $x++) {
        $idx = ($y * $gridW) + $x
        if ($dither[$idx] -eq 0) {
            $svgX = 65.0 + ($x / [double]$gridW) * 310.0
            $svgY = 110.0 + ($y / [double]$gridH) * 400.0
            $portraitPoints.Add((New-Object System.Drawing.PointF($svgX, $svgY)))
        }
    }
}

function Build-SVG($isDark) {
    $bg = if ($isDark) { "#0A101F" } else { "#F8FAFC" }
    $cardBg = if ($isDark) { "#0F172A" } else { "#FFFFFF" }
    $cardBorder = if ($isDark) { "#0891B2" } else { "#0284C7" }
    $cardHeaderBg = if ($isDark) { "#1E293B" } else { "#F1F5F9" }
    $portraitColor = if ($isDark) { "#A78BFA" } else { "#7C3AED" }
    $textColor = if ($isDark) { "#E2E8F0" } else { "#0F172A" }
    $labelColor = if ($isDark) { "#22D3EE" } else { "#0284C7" }
    $dimTextColor = if ($isDark) { "#94A3B8" } else { "#475569" }
    $accentColor = "#10B981"
    $badgeBg = if ($isDark) { "rgba(16, 185, 129, 0.15)" } else { "rgba(16, 185, 129, 0.1)" }
    
    $dotsSvg = New-Object System.Text.StringBuilder
    $numDots = $portraitPoints.Count
    $rnd = New-Object System.Random(44710)

    for ($i = 0; $i -lt $numDots; $i++) {
        $pt = $portraitPoints[$i]
        $px = $pt.X
        $py = $pt.Y

        $groupIndex = $i % 60
        $shimmerDelay = [math]::Round(($groupIndex / 60.0) * 3.2, 2)

        # Gemini Twin-Sparkle Target
        if ($i % 3 -ne 0) {
            $dist = ($rnd.NextDouble() * 85)
            $signX = if ($rnd.Next(2) -eq 0) { 1 } else { -1 }
            $signY = if ($rnd.Next(2) -eq 0) { 1 } else { -1 }
            if ($i % 2 -eq 0) {
                $gx = 220 + ($signX * $dist * 1.1)
                $gy = 310 + ($rnd.NextDouble() * 10 - 5)
            } else {
                $gx = 220 + ($rnd.NextDouble() * 10 - 5)
                $gy = 310 + ($signY * $dist * 1.1)
            }
        } else {
            $gx = 310 + ($rnd.NextDouble() * 35 - 17)
            $gy = 210 + ($rnd.NextDouble() * 35 - 17)
        }

        # Capital S Logo Target
        $sFrac = $i / [double]$numDots
        if ($sFrac -lt 0.35) {
            $t = $sFrac / 0.35
            $angle = [math]::PI * 0.15 + ($t * [math]::PI * 1.1)
            $sX = 220 + [math]::Cos($angle) * 75 + ($rnd.NextDouble() * 6 - 3)
            $sY = 220 - [math]::Sin($angle) * 55 + ($rnd.NextDouble() * 6 - 3)
        } elseif ($sFrac -lt 0.65) {
            $t = ($sFrac - 0.35) / 0.30
            $sX = 175 + ($t * 90) + ($rnd.NextDouble() * 6 - 3)
            $sY = 250 + ($t * 90) + ($rnd.NextDouble() * 6 - 3)
        } else {
            $t = ($sFrac - 0.65) / 0.35
            $angle = [math]::PI * 1.15 + ($t * [math]::PI * 1.1)
            $sX = 220 + [math]::Cos($angle) * 75 + ($rnd.NextDouble() * 6 - 3)
            $sY = 375 - [math]::Sin($angle) * 55 + ($rnd.NextDouble() * 6 - 3)
        }

        # GitHub Octocat Target
        $ghAngle = ($i / $numDots) * 2 * [math]::PI
        $ghR = 80 + ($rnd.NextDouble() * 15)
        $hX = 220 + [math]::Cos($ghAngle) * $ghR
        $hY = 300 + [math]::Sin($ghAngle) * $ghR
        if ($i % 12 -eq 0) {
            $hX = 160 + ($rnd.NextDouble() * 24 - 12)
            $hY = 200 + ($rnd.NextDouble() * 24 - 12)
        } elseif ($i % 12 -eq 1) {
            $hX = 280 + ($rnd.NextDouble() * 24 - 12)
            $hY = 200 + ($rnd.NextDouble() * 24 - 12)
        }

        $fPx = "{0:F1}" -f $px
        $fPy = "{0:F1}" -f $py
        $fGx = "{0:F1}" -f $gx
        $fGy = "{0:F1}" -f $gy
        $fSx = "{0:F1}" -f $sX
        $fSy = "{0:F1}" -f $sY
        $fHx = "{0:F1}" -f $hX
        $fHy = "{0:F1}" -f $hY

        $dotSize = "{0:F1}" -f (1.6 + ($rnd.NextDouble() * 1.0))

        [void]$dotsSvg.AppendLine(@"
        <circle class="diter-dot dot-grp-$groupIndex" cx="$fPx" cy="$fPy" r="$dotSize" fill="$portraitColor" opacity="0">
            <animate attributeName="opacity" values="0;0.95;0.9" keyTimes="0; 0.5; 1" dur="0.8s" begin="${shimmerDelay}s" fill="freeze" />
            <animate attributeName="cx" values="$fPx; $fPx; $fGx; $fGx; $fSx; $fSx; $fHx; $fHx; $fPx" keyTimes="0; 0.22; 0.28; 0.47; 0.53; 0.72; 0.78; 0.94; 1" dur="14.2s" begin="3.2s" repeatCount="indefinite" />
            <animate attributeName="cy" values="$fPy; $fPy; $fGy; $fGy; $fSy; $fSy; $fHy; $fHy; $fPy" keyTimes="0; 0.22; 0.28; 0.47; 0.53; 0.72; 0.78; 0.94; 1" dur="14.2s" begin="3.2s" repeatCount="indefinite" />
        </circle>
"@)
    }

    return @"
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1180 610" width="1180" height="610">
  <defs>
    <style>
      .bg-rect { fill: $bg; }
      .font-mono { font-family: Consolas, 'Courier New', monospace; }
      .font-sans { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; }
      
      .card-main { fill: $cardBg; stroke: $cardBorder; stroke-width: 1.5; rx: 12; }
      .card-header { fill: $cardHeaderBg; rx: 12; }
      .text-title { fill: $textColor; font-weight: 700; font-size: 15px; }
      .text-label { fill: $labelColor; font-weight: 700; font-size: 12.5px; letter-spacing: 0.5px; }
      .text-val { fill: $textColor; font-weight: 500; font-size: 12.5px; }
      .text-dim { fill: $dimTextColor; font-size: 11.5px; }
      .text-accent { fill: $accentColor; font-weight: 700; }
      
      @keyframes pulse-glow {
        0% { opacity: 0.4; }
        50% { opacity: 1; }
        100% { opacity: 0.4; }
      }
      .live-dot { fill: $accentColor; animation: pulse-glow 2s infinite ease-in-out; }
      .diter-dot { transition: all 0.3s ease; }
    </style>

    <linearGradient id="panel-grad" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" stop-color="$cardBg" stop-opacity="0.9" />
      <stop offset="100%" stop-color="$bg" stop-opacity="0.95" />
    </linearGradient>
    
    <linearGradient id="accent-line" x1="0%" y1="0%" x2="100%" y2="0%">
      <stop offset="0%" stop-color="$labelColor" />
      <stop offset="50%" stop-color="$portraitColor" />
      <stop offset="100%" stop-color="$accentColor" />
    </linearGradient>
  </defs>

  <!-- Background Canvas -->
  <rect class="bg-rect" width="1180" height="610" rx="16" />
  <rect x="2" y="2" width="1176" height="606" fill="none" stroke="url(#accent-line)" stroke-width="1.5" rx="15" opacity="0.4" />

  <!-- LEFT PANEL: Animated Dithered Portrait & S Logo Morph Frame -->
  <g transform="translate(10, 0)">
    <rect x="35" y="45" width="370" height="520" rx="14" fill="url(#panel-grad)" stroke="$cardBorder" stroke-width="1" opacity="0.6" />
    
    <path d="M 45 65 L 45 45 L 65 45" fill="none" stroke="$labelColor" stroke-width="2.5" />
    <path d="M 395 65 L 395 45 L 375 45" fill="none" stroke="$labelColor" stroke-width="2.5" />
    <path d="M 45 545 L 45 565 L 65 565" fill="none" stroke="$labelColor" stroke-width="2.5" />
    <path d="M 395 545 L 395 565 L 375 565" fill="none" stroke="$labelColor" stroke-width="2.5" />

    <text x="55" y="75" class="font-mono text-label" font-size="11" opacity="0.8">SYSTEM.PORTRAIT // FLOYD-STEINBERG</text>
    <text x="310" y="75" class="font-mono text-dim" font-size="10">300x340</text>

    <!-- Photo-Generated Dithered Dots Container -->
    <g id="portrait-dots">
      $($dotsSvg.ToString())
    </g>

    <!-- Morph Stage Identifier Pill -->
    <rect x="65" y="525" width="310" height="24" rx="12" fill="$badgeBg" stroke="$accentColor" stroke-width="1" opacity="0.8" />
    <text x="220" y="541" class="font-mono text-accent" font-size="11" text-anchor="middle">
      <tspan>MORPH: PORTRAIT --&gt; GEMINI --&gt; S-LOGO --&gt; GITHUB</tspan>
    </text>
  </g>

  <!-- RIGHT PANEL: SYSTEM.INFO Terminal Card UI -->
  <g transform="translate(435, 45)">
    <rect class="card-main" x="0" y="0" width="705" height="520" />
    
    <rect class="card-header" x="0" y="0" width="705" height="42" />
    <rect x="0" y="41" width="705" height="1" fill="$cardBorder" opacity="0.4" />
    
    <circle cx="22" cy="21" r="5.5" fill="#EF4444" opacity="0.85" />
    <circle cx="39" cy="21" r="5.5" fill="#F59E0B" opacity="0.85" />
    <circle cx="56" cy="21" r="5.5" fill="#10B981" opacity="0.85" />
    
    <text x="78" y="26" class="font-mono text-title">profile.sh --live</text>
    
    <g transform="translate(595, 11)">
      <rect x="0" y="0" width="88" height="22" rx="11" fill="$badgeBg" stroke="$accentColor" stroke-width="1" />
      <circle class="live-dot" cx="14" cy="11" r="4" />
      <text x="26" y="15" class="font-mono text-accent" font-size="11">LIVE</text>
    </g>

    <!-- SYSTEM.INFO Terminal Grid Content -->
    <g transform="translate(25, 62)">
      <!-- Line 1: Subject -->
      <text x="0" y="18" class="font-mono text-label">Subject</text>
      <text x="140" y="18" class="font-sans text-val" font-weight="700" font-size="15" fill="$labelColor">SAKTHIPRAKASH T</text>
      
      <!-- Line 2: Role -->
      <text x="0" y="44" class="font-mono text-label">Role</text>
      <text x="140" y="44" class="font-sans text-val">AI Engineer | AI Developer | Hackathon Builder</text>

      <!-- Line 3: Origin & Education -->
      <text x="0" y="70" class="font-mono text-label">Origin</text>
      <text x="140" y="70" class="font-sans text-val">Puducherry, India</text>
      <text x="340" y="70" class="font-mono text-label">Education</text>
      <text x="430" y="70" class="font-sans text-val">B.Tech IT - SMVEC</text>

      <!-- Line 4: Status -->
      <text x="0" y="96" class="font-mono text-label">Status</text>
      <text x="140" y="96" class="font-sans text-accent">[ACTIVE] Building - Learning - Shipping</text>

      <line x1="0" y1="112" x2="655" y2="112" stroke="$cardBorder" stroke-width="1" stroke-dasharray="4 4" opacity="0.3" />

      <!-- TECH STACK MATRIX -->
      <text x="0" y="136" class="font-mono text-label">ToolChain</text>
      <text x="140" y="136" class="font-sans text-val">VS Code | Cursor AI | Git | Firebase Studio | Docker</text>

      <text x="0" y="162" class="font-mono text-label">Languages</text>
      <text x="140" y="162" class="font-sans text-val">Python | Java | JavaScript | TypeScript | C | SQL</text>

      <text x="0" y="188" class="font-mono text-label">Frontend</text>
      <text x="140" y="188" class="font-sans text-val">React | Next.js | Tailwind CSS | HTML5 | CSS3</text>

      <text x="0" y="214" class="font-mono text-label">Backend</text>
      <text x="140" y="214" class="font-sans text-val">FastAPI | Flask | Node.js | Express.js</text>

      <text x="0" y="240" class="font-mono text-label">Database</text>
      <text x="140" y="240" class="font-sans text-val">PostgreSQL | MongoDB | Firebase Firestore | SQLite | ChromaDB</text>

      <text x="0" y="266" class="font-mono text-label">Core AI</text>
      <text x="140" y="266" class="font-sans text-val" fill="$portraitColor" font-weight="700">Google Gemini API | Vertex AI | Firebase AI | LangChain | Ollama | RAG</text>

      <text x="0" y="292" class="font-mono text-label">Cloud / Infra</text>
      <text x="140" y="292" class="font-sans text-val">Google Cloud | Firebase | Docker | Vercel | GitHub Actions</text>

      <line x1="0" y1="308" x2="655" y2="308" stroke="$cardBorder" stroke-width="1" stroke-dasharray="4 4" opacity="0.3" />

      <!-- CONTACT DIRECTORY -->
      <text x="0" y="332" class="font-mono text-label">Mail</text>
      <text x="140" y="332" class="font-mono text-val" font-size="12">sakthiprakashthangaraj@gmail.com</text>

      <text x="0" y="358" class="font-mono text-label">Portfolio</text>
      <text x="140" y="358" class="font-mono text-val" font-size="12" fill="$labelColor">portfolio-six-wheat-wig4lpjmp7.vercel.app</text>

      <text x="0" y="384" class="font-mono text-label">LinkedIn</text>
      <text x="140" y="384" class="font-mono text-val" font-size="12">linkedin.com/in/sakthi-prakash-44710</text>

      <text x="0" y="410" class="font-mono text-label">GitHub</text>
      <text x="140" y="410" class="font-mono text-val" font-size="12">github.com/sakthi44710 (@sakthi44710)</text>
      
      <text x="0" y="442" class="font-mono text-dim" font-size="11">&gt; SYSTEM STATUS: ONLINE // AI OS ACTIVE // READY TO SHIP</text>
    </g>
  </g>
</svg>
"@
}

# Save using clean ASCII / UTF8 without BOM
$utf8NoBom = New-Object System.Text.UTF8Encoding($false)
[System.IO.File]::WriteAllText($darkSvgPath, (Build-SVG -isDark $true), $utf8NoBom)
[System.IO.File]::WriteAllText($lightSvgPath, (Build-SVG -isDark $false), $utf8NoBom)

Write-Output "BULLETPROOF_CLEAN_TEXT_SUCCESS: Banners regenerated with 100% clean ASCII text!"
